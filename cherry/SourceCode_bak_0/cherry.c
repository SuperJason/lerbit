...
#define LERBIT_BOARD 0 /* 0: debug board, 1: cherry board */
#define LERBIT_BUZZER

#define MAX_RTC_TASKS_DELAY               47                                                /**< Maximum delay until an RTC task is executed. */

#if LERBIT_BOARD == 0
#define DEVICE_NAME                       "CHERRY_0"                                        /**< Name of device. Will be included in the advertising data. */
#else
#define DEVICE_NAME                       "CHERRY_1"                                        /**< Name of device. Will be included in the advertising data. */
#endif
...
#define BUTTON_TIMER_INTERVAL             APP_TIMER_TICKS(1000, APP_TIMER_PRESCALER)        /**< Button long press interval (ticks). */
...
#else
#define LERBIT_CHERRY_LED_PIN_NO               18
#define LERBIT_CHERRY_BUZZER_PIN_NO            6
#define LERBIT_CHERRY_BUTTON_PIN_NO            16
#endif
...
typedef enum {
  LERBIT_IAS_SRV_NOT_FOUND = 0,
  LERBIT_IAS_SRV_DISCOVERED,
  LERBIT_IAS_DISCONN_COMPLETE
} LERBIT_IAS_STATUS;
...
static app_timer_id_t                     m_button_timer_id;                                /**< Long press timer. */
...
unsigned int lerbit_cherry_led_flash_cnt = 5;
unsigned int lerbit_cherry_pwm_led_max_sample = L_CLOCK_ONE_HZ_SAMPLE * 5;
unsigned int lerbit_cherry_pwm_led_sample = L_CLOCK_ONE_HZ_SAMPLE / 8;
unsigned int lerbit_cherry_pwm_buzzer_max_sample = L_CLOCK_ONE_HZ_SAMPLE;
#if LERBIT_BOARD == 0
unsigned int lerbit_cherry_pwm_buzzer_sample =  L_CLOCK_ONE_HZ_SAMPLE / 2;
#else
unsigned int lerbit_cherry_pwm_buzzer_sample =  4;
#endif
unsigned int lerbit_button_long_pressed = 0;
unsigned int lerbit_cherry_led_flash_cnt_temp = 0;
unsigned int lerbit_connect_status = 0;
LERBIT_IAS_STATUS lerbit_ias_c_status = LERBIT_IAS_SRV_NOT_FOUND;

LERBIT_LED_STATUS lerbit_cherry_led_current_status;
...
void app_error_handler(uint32_t error_code, uint32_t line_num, const uint8_t * p_file_name)
{
  ...
  lerbit_cherry_led_contrl(LERBIT_LED_Asserted);
...
#ifdef LERBIT_BUZZER
static void lerbit_cherry_buzzer_enable(void)
{
  nrf_gpiote_task_config(1, LERBIT_CHERRY_BUZZER_PIN_NO, GPIOTE_CONFIG_POLARITY_Toggle, NRF_GPIOTE_INITIAL_VALUE_LOW);
...
static void lerbit_cherry_buzzer_disable(void)
{
  // Stop clock
  NRF_TIMER2->TASKS_STOP = 1;
  NRF_GPIOTE->TASKS_OUT[1] = 1;
  nrf_gpiote_unconfig(1);
  nrf_gpio_pin_clear(LERBIT_CHERRY_BUZZER_PIN_NO);
}
#endif
...
static void advertising_start(void)
{
  ...
  lerbit_cherry_led_contrl(LERBIT_LED_turn_off);
  if (!m_is_link_loss_alerting)
  {
#ifdef LERBIT_BUZZER
    lerbit_cherry_buzzer_disable();
#endif 
  }
  ...
  switch (m_advertising_mode)
  {
    ...
    case BLE_FAST_ADV_WHITELIST:
      ...
      if ((whitelist.addr_count != 0) || (whitelist.irk_count != 0))
      {
        ...
        lerbit_cherry_led_contrl(LERBIT_LED_Whitelist_Advertising);
      } else {
        ...
        adv_params.timeout  = ADV_INTERVAL_FAST_PERIOD;
        m_advertising_mode  = BLE_FAST_ADV;
        lerbit_cherry_led_contrl(LERBIT_LED_Advertising);
      }
      break;
    ...
    case BLE_SLOW_ADV:
      ...
      lerbit_cherry_led_contrl(LERBIT_LED_Slow_Advertising);
      break;
...
static void on_ias_c_evt(ble_ias_c_t * p_ias_c, ble_ias_c_evt_t * p_evt)
{
  ...
  switch (p_evt->evt_type)
  {
    case BLE_IAS_C_EVT_SRV_DISCOVERED:
      ...
      //lerbit_cherry_led_contrl(LERBIT_LED_Immediate_Alert);
      lerbit_ias_c_status = LERBIT_IAS_SRV_DISCOVERED;
      break;

    case BLE_IAS_C_EVT_SRV_NOT_FOUND:
      // IAS is not found on peer.
      lerbit_ias_c_status = LERBIT_IAS_SRV_NOT_FOUND;
      break;

    case BLE_IAS_C_EVT_DISCONN_COMPLETE:
      //nrf_gpio_pin_clear(PEER_SRV_DISC_LED_PIN_NO);
      lerbit_ias_c_status = LERBIT_IAS_DISCONN_COMPLETE;
      break;
    ...
    case BLE_GAP_EVT_TIMEOUT:
      ...
      { 
        if (m_advertising_mode == BLE_SLEEP)
        {
          ...
          GPIO_WAKEUP_BUTTON_WITH_PULLUP_CONFIG(LERBIT_CHERRY_BUTTON_PIN_NO);
#if LERBIT_BOARD == 0
          nrf_gpio_pin_set(10);
#endif
...
*/
static void bond_manager_init(bool bonds_delete)
{
  uint32_t            err_code;
...
void lerbit_rtc1_irq_handler(void)
{
  ...
  if (NRF_RTC1->EVENTS_COMPARE[2] != 0) {
    ...
    nrf_gpio_pin_set(LERBIT_CHERRY_LED_PIN_NO);
    lerbit_cherry_led_flash_cnt_temp = (lerbit_cherry_led_flash_cnt - 1) * 2;
    NRF_RTC1->CC[3] = NRF_RTC1->CC[2] + lerbit_cherry_pwm_led_sample;
    NRF_RTC1->CC[2] += lerbit_cherry_pwm_led_max_sample;
  }

  if (NRF_RTC1->EVENTS_COMPARE[3] != 0) {
    NRF_RTC1->EVENTS_COMPARE[3] = 0;
    if (lerbit_cherry_led_flash_cnt_temp) {
      if (lerbit_cherry_led_flash_cnt_temp % 2)
        NRF_RTC1->CC[3] += lerbit_cherry_pwm_led_sample;
      else 
        NRF_RTC1->CC[3] += lerbit_cherry_pwm_led_sample * 3;
      lerbit_cherry_led_flash_cnt_temp--;
    }
    nrf_gpio_pin_toggle(LERBIT_CHERRY_LED_PIN_NO);
  }
...
#if LERBIT_BOARD == 0
  NRF_TIMER2->PRESCALER = 9; // 1Hz
#else
  NRF_TIMER2->PRESCALER = 4; // 4kHz
#endif
...
#if LERBIT_BOARD == 0
  NRF_TIMER2->CC[0] = 31250UL; // 1Hz
  NRF_TIMER2->CC[1] = 15625UL;
#else
  NRF_TIMER2->CC[0] = 250; //4kHz
  NRF_TIMER2->CC[1] = 125;
#endif

  //PPI Toggle buzzer pin
  NRF_PPI->CH[0].EEP = (uint32_t)&NRF_TIMER2->EVENTS_COMPARE[1];
...
static void lerbit_cherry_led_contrl(LERBIT_LED_STATUS led_status)
{
  switch(led_status)
  {
    case LERBIT_LED_Advertising:
      lerbit_cherry_led_current_status = LERBIT_LED_Advertising;

      lerbit_cherry_led_flash_cnt = 1;
      lerbit_cherry_pwm_led_sample = L_CLOCK_ONE_HZ_SAMPLE / 16;

      break;
    case LERBIT_LED_Connected:
      if (lerbit_connect_status) {
        lerbit_cherry_led_current_status = LERBIT_LED_Connected;

        lerbit_cherry_led_flash_cnt = 2;
        lerbit_cherry_pwm_led_sample = L_CLOCK_ONE_HZ_SAMPLE / 16;
      }

      break;
    case LERBIT_LED_Mild_Alert:
      lerbit_cherry_led_current_status = LERBIT_LED_Mild_Alert;

      lerbit_cherry_led_flash_cnt = 3;
      lerbit_cherry_pwm_led_sample = L_CLOCK_ONE_HZ_SAMPLE / 16;

      break;
    case LERBIT_LED_High_Alert:
      lerbit_cherry_led_current_status = LERBIT_LED_High_Alert;

      lerbit_cherry_led_flash_cnt = 4;
      lerbit_cherry_pwm_led_sample = L_CLOCK_ONE_HZ_SAMPLE / 16;

      break;
    case LERBIT_LED_Slow_Advertising:
      lerbit_cherry_led_current_status = LERBIT_LED_Slow_Advertising;

      lerbit_cherry_led_flash_cnt = 1;
      lerbit_cherry_pwm_led_sample = L_CLOCK_ONE_HZ_SAMPLE / 8;

      break;
    case LERBIT_LED_Immediate_Alert:
      lerbit_cherry_led_current_status = LERBIT_LED_Immediate_Alert;
    ...
    case LERBIT_LED_Whitelist_Advertising:
      lerbit_cherry_led_current_status = LERBIT_LED_Whitelist_Advertising;
    ...
    case LERBIT_LED_Asserted:
      lerbit_cherry_led_current_status = LERBIT_LED_Asserted;

      lerbit_cherry_pwm_led_sample = L_CLOCK_ONE_HZ_SAMPLE / 8;
      lerbit_cherry_led_flash_cnt = 4;

      break;
    case LERBIT_LED_turn_off:
      lerbit_cherry_led_current_status = LERBIT_LED_turn_off;

      nrf_gpio_pin_clear(LERBIT_CHERRY_LED_PIN_NO);
      break;
    default:
      break;
  }

  if (LERBIT_LED_turn_off == lerbit_cherry_led_current_status) {
    NRF_RTC1->INTENCLR = (RTC_EVTEN_COMPARE2_Enabled << RTC_EVTEN_COMPARE2_Pos)
      | (RTC_EVTEN_COMPARE3_Enabled << RTC_EVTEN_COMPARE3_Pos);
    NRF_RTC1->EVTENCLR = (RTC_EVTEN_COMPARE2_Enabled << RTC_EVTEN_COMPARE2_Pos)
      | (RTC_EVTEN_COMPARE3_Enabled << RTC_EVTEN_COMPARE3_Pos);
  } else {
    NRF_RTC1->CC[2] = NRF_RTC1->COUNTER + lerbit_cherry_pwm_led_max_sample ;
    NRF_RTC1->INTENSET = (RTC_EVTEN_COMPARE2_Enabled << RTC_EVTEN_COMPARE2_Pos)
      | (RTC_EVTEN_COMPARE3_Enabled << RTC_EVTEN_COMPARE3_Pos);
    NRF_RTC1->EVTENSET = (RTC_EVTEN_COMPARE2_Enabled << RTC_EVTEN_COMPARE2_Pos)
      | (RTC_EVTEN_COMPARE3_Enabled << RTC_EVTEN_COMPARE3_Pos);
  }
...
static void button_timeout_handler(void * p_context)
{
  ...
  switch(lerbit_button_long_pressed)
  {
    case 1:
      if (0 == nrf_gpio_pin_read(LERBIT_CHERRY_BUTTON_PIN_NO)) {
        lerbit_cherry_led_contrl(LERBIT_LED_High_Alert);
        // Long Press
...
static void button_event_handler(uint8_t pin_no)
{
  ...
  switch (pin_no)
  {
    case LERBIT_CHERRY_BUTTON_PIN_NO:
      if (0 == lerbit_button_long_pressed ) {
        // Start battery timer
        err_code = app_timer_start(m_button_timer_id, BUTTON_TIMER_INTERVAL, NULL);
        APP_ERROR_CHECK(err_code);
      }
      if (lerbit_button_long_pressed < 8)
        lerbit_button_long_pressed++;
#if 1
      if (lerbit_cherry_led_current_status != LERBIT_LED_turn_off) {
        lerbit_cherry_led_contrl(lerbit_cherry_led_current_status + 1);
#ifdef LERBIT_BUZZER
        lerbit_cherry_buzzer_disable();
#endif
      } else {
      ...
      if ((lerbit_ias_c_status == LERBIT_IAS_SRV_DISCOVERED)) {
...
{
  //GPIO
  GPIO_LED_CONFIG(LERBIT_CHERRY_LED_PIN_NO);
...
int main(void)
{
  ...
  if (0 == nrf_gpio_pin_read(LERBIT_CHERRY_BUTTON_PIN_NO)) {
    bond_manager_init(true);
  } else {
    bond_manager_init(false);
  }
  ble_stack_init();
  gap_params_init();
  advertising_init(BLE_GAP_ADV_FLAGS_LE_ONLY_LIMITED_DISC_MODE);
