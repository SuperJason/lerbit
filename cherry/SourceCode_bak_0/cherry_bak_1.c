...
#define LERBIT_BOARD 1 /* 0: debug board, 1: cherry board */
...
static app_timer_id_t                     m_led_timer_id;                                   /**< Led timer. */
#ifdef LERBIT_BUZZER
static app_timer_id_t                     m_buzzer_timer_id;                                   /**< Led timer. */
#endif
...
static void lerbit_cherry_buzzer_enable(void)
{
		uint32_t err_code;
#if LERBIT_BOARD == 0
	  err_code = app_timer_start(m_buzzer_timer_id, L_CLOCK_ONE_HZ_SAMPLE, NULL);
#else
    err_code = app_timer_start(m_buzzer_timer_id, 8, NULL); // 4K
#endif
...
static void led_timeout_handler(void * p_context)
{
    ...
    NRF_RTC1->CC[3] = NRF_RTC1->COUNTER + lerbit_cherry_pwm_led_sample;
	  NRF_RTC1->INTENSET = (RTC_EVTEN_COMPARE3_Enabled << RTC_EVTEN_COMPARE3_Pos);
}
...
static void buzzer_timeout_handler(void * p_context)
{
    UNUSED_PARAMETER(p_context);
	  ...
	  nrf_gpio_pin_set(LERBIT_CHERRY_BUZZER_PIN_NO);
	  NRF_RTC1->CC[2] = NRF_RTC1->COUNTER + lerbit_cherry_pwm_buzzer_sample;
...
static void lerbit_pwm_init(void)
{
    //GPIO
	  ...
	  //PPI
    NRF_PPI->CH[0].EEP = (uint32_t)&NRF_RTC1->EVENTS_COMPARE[2];
  ...
  switch(led_status)
  {
    case LERBIT_LED_Advertising:
      ...
      lerbit_cherry_pwm_led_sample = L_CLOCK_ONE_HZ_SAMPLE / 16;
		  ...
		  err_code = app_timer_start(m_led_timer_id, APP_TIMER_TICKS(3000, APP_TIMER_PRESCALER), NULL);
      APP_ERROR_CHECK(err_code);
    ...
    case LERBIT_LED_Slow_Advertising:
      ...
      err_code = app_timer_start(m_led_timer_id, APP_TIMER_TICKS(5000, APP_TIMER_PRESCALER), NULL);
      APP_ERROR_CHECK(err_code);
    ...
    case LERBIT_LED_Asserted:
      ...
      break;
		case LERBIT_LED_turn_off:
		  ...
		  NRF_RTC1->INTENCLR = (RTC_EVTEN_COMPARE3_Enabled << RTC_EVTEN_COMPARE3_Pos);
			break;
    default:
      break;
  }
...
#if 0
            if (lerbit_cherry_led_current_status != LERBIT_LED_Asserted)
...
static void leds_init(void)
{
    ...
    GPIO_LED_CONFIG(LERBIT_CHERRY_BUZZER_PIN_NO);
}
